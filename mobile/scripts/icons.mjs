...
mkdirSync(assetsDir);
// The icon fills 70% of the square so its own rounded corners sit inside the
// rounding the phone applies, instead of being clipped by it.
await tile(1024, 720, 'icon-only.png');
// Android adaptive icons: the foreground must fit the middle 66% safe zone.
await tile(1024, 560, 'icon-foreground.png', true);
await sharp({ create: { width: 1024, height: 1024, channels: 3, background: BACKGROUND } })
  .png()
