...
Uploads are limited to signature-checked images and PDFs, with a default maximum size of 100 MB (`MAX_UPLOAD_BYTES` can change the limit). SQLite and local uploads require one application instance. Presence and whiteboard sessions are held in memory and are lost on restart. In production, configure Twilio Network Traversal with `TWILIO_ACCOUNT_SID`, `TWILIO_API_KEY`, and `TWILIO_API_SECRET` Railway secrets. The authenticated `/api/ice-config` endpoint requests one-hour STUN/TURN credentials from Twilio for each client; credentials are not stored in the browser bundle. Local development falls back to public STUN when Twilio settings are absent. TURN relays are billed by bandwidth usage.
